...
operator fun Matrix3f.minusAssign(m: Matrix3f) {
    sub(m)
}

operator fun Matrix3fc.minus(m: Matrix3f) = sub(m, Matrix3f())
operator fun Matrix3f.plusAssign(m: Matrix3fc) {
    add(m)
}

operator fun Matrix3fc.plus(m: Matrix3fc) = add(m, Matrix3f())
operator fun Matrix3f.timesAssign(m: Matrix3fc) {
    mul(m)
}

operator fun Matrix3fc.times(m: Matrix3fc) = mul(m, Matrix3f())
operator fun Matrix3f.timesAssign(v: Vector3f) {
    transform(v)
}

operator fun Matrix3fc.times(v: Vector3f) = transform(v, Vector3f())
operator fun Matrix3f.timesAssign(q: Quaternionfc) {
    rotate(q)
}

operator fun Matrix3fc.times(q: Quaternionfc) = rotate(q, Matrix3f())
infix fun Matrix3f.rotate(q: Quaternionfc) {
    rotate(q)
}

infix fun Matrix3fc.transform(v: Vector3f) {
    transform(v)
}
...
operator fun Matrix3d.minusAssign(m: Matrix3d) {
    sub(m)
}

operator fun Matrix3dc.minus(m: Matrix3d) = sub(m, Matrix3d())
operator fun Matrix3d.plusAssign(m: Matrix3dc) {
    add(m)
}

operator fun Matrix3dc.plus(m: Matrix3dc) = add(m, Matrix3d())
operator fun Matrix3d.timesAssign(m: Matrix3dc) {
    mul(m)
}

operator fun Matrix3dc.times(m: Matrix3dc) = mul(m, Matrix3d())
operator fun Matrix3d.timesAssign(m: Matrix3fc) {
    mul(m)
}

operator fun Matrix3dc.times(m: Matrix3fc) = mul(m, Matrix3d())
operator fun Matrix3d.timesAssign(v: Vector3d) {
    transform(v)
}

operator fun Matrix3dc.times(v: Vector3d) = transform(v, Vector3d())
operator fun Matrix3d.timesAssign(v: Vector3f) {
    transform(v)
}

operator fun Matrix3dc.times(v: Vector3f) = transform(v, Vector3f())
operator fun Matrix3d.timesAssign(q: Quaternionfc) {
    rotate(q)
}

operator fun Matrix3dc.times(q: Quaternionfc) = rotate(q, Matrix3d())
operator fun Matrix3d.timesAssign(q: Quaterniondc) {
    rotate(q)
}

operator fun Matrix3dc.times(q: Quaterniondc) = rotate(q, Matrix3d())
infix fun Matrix3d.rotate(q: Quaternionfc) {
    rotate(q)
}

infix fun Matrix3d.rotate(q: Quaterniondc) {
    rotate(q)
}

infix fun Matrix3dc.transform(v: Vector3f) {
    transform(v)
}

infix fun Matrix3dc.transform(v: Vector3d) {
    transform(v)
}
...
operator fun Matrix4x3f.minusAssign(m: Matrix4x3f) {
    sub(m)
}

operator fun Matrix4x3fc.minus(m: Matrix4x3f) = sub(m, Matrix4x3f())
operator fun Matrix4x3f.plusAssign(m: Matrix4x3fc) {
    add(m)
}

operator fun Matrix4x3fc.plus(m: Matrix4x3fc) = add(m, Matrix4x3f())
operator fun Matrix4x3f.timesAssign(m: Matrix4x3fc) {
    mul(m)
}

operator fun Matrix4x3fc.times(m: Matrix4x3fc) = mul(m, Matrix4x3f())
operator fun Matrix4x3f.timesAssign(v: Vector4f) {
    transform(v)
}

operator fun Matrix4x3fc.times(v: Vector4f) = transform(v, Vector4f())
operator fun Matrix4x3f.timesAssign(q: Quaternionfc) {
    rotate(q)
}

operator fun Matrix4x3fc.times(q: Quaternionfc) = rotate(q, Matrix4x3f())
infix fun Matrix4x3f.rotate(q: Quaternionfc) {
    rotate(q)
}

infix fun Matrix4x3fc.transform(v: Vector4f) {
    transform(v)
}

infix fun Matrix4x3fc.transformPosition(v: Vector3f) {
    transformPosition(v)
}

infix fun Matrix4x3fc.transformDirection(v: Vector3f) {
...
operator fun Matrix4x3d.minusAssign(m: Matrix4x3dc) {
    sub(m)
}

operator fun Matrix4x3dc.minus(m: Matrix4x3dc) = sub(m, Matrix4x3d())
operator fun Matrix4x3d.plusAssign(m: Matrix4x3dc) {
    add(m)
}

operator fun Matrix4x3dc.plus(m: Matrix4x3dc) = add(m, Matrix4x3d())
operator fun Matrix4x3d.timesAssign(m: Matrix4x3fc) {
    mul(m)
}

operator fun Matrix4x3dc.times(m: Matrix4x3fc) = mul(m, Matrix4x3d())
operator fun Matrix4x3d.timesAssign(m: Matrix4x3dc) {
    mul(m)
}

operator fun Matrix4x3dc.times(m: Matrix4x3dc) = mul(m, Matrix4x3d())
operator fun Matrix4x3d.timesAssign(v: Vector4d) {
    transform(v)
}

operator fun Matrix4x3dc.times(v: Vector4d) = transform(v, Vector4d())
operator fun Matrix4x3d.timesAssign(q: Quaternionfc) {
    rotate(q)
}

operator fun Matrix4x3dc.times(q: Quaternionfc) = rotate(q, Matrix4x3d())
operator fun Matrix4x3d.timesAssign(q: Quaterniondc) {
    rotate(q)
}

operator fun Matrix4x3dc.times(q: Quaterniondc) = rotate(q, Matrix4x3d())
infix fun Matrix4x3d.rotate(q: Quaternionfc) {
    rotate(q)
}

infix fun Matrix4x3d.rotate(q: Quaterniondc) {
    rotate(q)
}

infix fun Matrix4x3dc.transform(v: Vector4d) {
    transform(v)
}

infix fun Matrix4x3dc.transformPosition(v: Vector3d) {
    transformPosition(v)
}

infix fun Matrix4x3dc.transformDirection(v: Vector3d) {
    transformDirection(v)
}
...
operator fun Matrix4f.minusAssign(m: Matrix4f) {
    sub(m)
}

operator fun Matrix4fc.minus(m: Matrix4f) = sub(m, Matrix4f())
operator fun Matrix4f.plusAssign(m: Matrix4fc) {
    add(m)
}

operator fun Matrix4fc.plus(m: Matrix4fc) = add(m, Matrix4f())
operator fun Matrix4f.timesAssign(m: Matrix4fc) {
    mul(m)
}

operator fun Matrix4fc.times(m: Matrix4fc) = mul(m, Matrix4f())
operator fun Matrix4f.timesAssign(m: Matrix4x3fc) {
    mul(m, this)
}

operator fun Matrix4f.timesAssign(v: Vector4f) {
    transform(v)
}

operator fun Matrix4fc.times(v: Vector4f) = transform(v, Vector4f())
operator fun Matrix4f.timesAssign(q: Quaternionfc) {
    rotate(q)
}

operator fun Matrix4fc.times(q: Quaternionfc) = rotate(q, Matrix4f())
infix fun Matrix4f.mulAffine(m: Matrix4fc) {
    this.mulAffine(m)
}

infix fun Matrix4f.mulAffineR(m: Matrix4fc) {
    this.mulAffineR(m)
}
...
infix fun Matrix4f.transform(v: Vector4f) {
    transform(v)
}

infix fun Matrix4f.transformPosition(v: Vector3f) {
    transformPosition(v)
}

infix fun Matrix4f.transformDirection(v: Vector3f) {
    transformDirection(v)
}
...
operator fun Matrix4d.minusAssign(m: Matrix4dc) {
    sub(m)
}

operator fun Matrix4dc.minus(m: Matrix4dc) = sub(m, Matrix4d())
operator fun Matrix4d.plusAssign(m: Matrix4dc) {
    add(m)
}

operator fun Matrix4dc.plus(m: Matrix4dc) = add(m, Matrix4d())
operator fun Matrix4d.timesAssign(m: Matrix4dc) {
    mul(m)
}

operator fun Matrix4dc.times(m: Matrix4dc) = mul(m, Matrix4d())
operator fun Matrix4d.timesAssign(m: Matrix4x3fc) {
    mul(m, this)
}

operator fun Matrix4d.timesAssign(m: Matrix4x3dc) {
    mul(m, this)
}

operator fun Matrix4d.timesAssign(v: Vector4d) {
    transform(v)
}

operator fun Matrix4dc.times(v: Vector4d) = transform(v, Vector4d())
operator fun Matrix4d.timesAssign(q: Quaternionfc) {
    rotate(q)
}

operator fun Matrix4dc.times(q: Quaternionfc) = rotate(q, Matrix4d())
operator fun Matrix4d.timesAssign(q: Quaterniondc) {
    rotate(q)
}

operator fun Matrix4dc.times(q: Quaterniondc) = rotate(q, Matrix4d())
infix fun Matrix4d.mulAffine(m: Matrix4dc) {
    this.mulAffine(m)
}

infix fun Matrix4d.mulAffineR(m: Matrix4dc) {
    this.mulAffineR(m)
}
...
infix fun Matrix4d.transform(v: Vector4d) {
    transform(v)
}

infix fun Matrix4dc.transformPosition(v: Vector3d) {
    transformPosition(v)
}

infix fun Matrix4dc.transformDirection(v: Vector3f) {
    transformDirection(v)
}

infix fun Matrix4dc.transformDirection(v: Vector3d) {
    transformDirection(v)
}
...
operator fun Vector2f.minusAssign(v: Vector2fc) {
    sub(v)
}

operator fun Vector2fc.minus(v: Vector2fc) = sub(v, Vector2f())
operator fun Vector2f.plusAssign(v: Vector2fc) {
    add(v)
}
...
operator fun Vector3f.minusAssign(v: Vector3fc) {
    sub(v)
}

operator fun Vector3fc.minus(v: Vector3fc) = sub(v, Vector3f())
operator fun Vector3f.plusAssign(v: Vector3fc) {
    add(v)
}
...
operator fun Vector3d.minusAssign(v: Vector3fc) {
    sub(v)
}

operator fun Vector3dc.minus(v: Vector3fc) = sub(v, Vector3d())
operator fun Vector3d.minusAssign(v: Vector3dc) {
    sub(v)
}

operator fun Vector3dc.minus(v: Vector3dc) = sub(v, Vector3d())
operator fun Vector3d.plusAssign(v: Vector3fc) {
    add(v)
}

operator fun Vector3dc.plus(v: Vector3fc) = add(v, Vector3d())
operator fun Vector3d.plusAssign(v: Vector3dc) {
    add(v)
}
...
operator fun Vector4f.minusAssign(v: Vector4fc) {
    sub(v)
}

operator fun Vector4fc.minus(v: Vector4fc) = sub(v, Vector4f())
operator fun Vector4f.plusAssign(v: Vector4fc) {
    add(v)
}
...
operator fun Quaternionf.minusAssign(q: Quaternionfc) {
    mul(q)
}
...
operator fun Quaternionf.timesAssign(v: Vector3f) {
    transform(v)
}

operator fun Quaternionfc.times(v: Vector3f) = transform(v, Vector3f())
operator fun Quaternionf.timesAssign(v: Vector4f) {
    transform(v)
}
...
operator fun Quaterniond.minusAssign(q: Quaterniondc) {
    mul(q)
}

operator fun Quaterniondc.minus(q: Quaterniondc) = mul(q, Quaterniond())
operator fun Quaterniond.unaryMinus() {
    conjugate()
}

operator fun Quaterniond.timesAssign(v: Vector3d) {
    transform(v)
}

operator fun Quaterniondc.times(v: Vector3d) = transform(v, Vector3d())
operator fun Quaterniond.timesAssign(v: Vector4d) {
    transform(v)
}
